...
ticker = 'X:TFUELUSD'
tiker_live = 'TFUEL/USD'
proxies = {'http': 'http://198.59.191.234:8080'}
#----- initialistion de l'API key et ticker -----#
...
#
                ...
                print('--- Mode recherche TFUEL',time1,time_name1,' ---', flush=True)
                ...
                if (C - B) < (C - D) and (C - B) < (E - D) and (E - F) < (E - D) and (E - F) < (
                        C - D) and B > D and F > D and B < C and F < E and A >= mirande2['c'].iloc[0] and verif == 0:
                        try:
                                J = line_intersection((AJ, BJ), (CJ, DJ))
                                I = line_intersection((AI, BI), (CI, DI))
                                accept = True
